...
export default function Edit({ recipient }: Props) {
    const { data, setData, patch, processing, errors } = useForm<RecipientForm>({
        id: recipient.id,
        name: recipient.name,
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        patch(route('recipients.update', { id: recipient.id }));
    };

    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold dark:text-white">Edit Recipient</h2>}
        >
